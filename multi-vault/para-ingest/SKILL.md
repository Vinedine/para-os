...
**It never decides what an item means.** No classifying, no drafting an action, no filing into an entity, no touching a mailbox. Every judgment about meaning stays with `/para-triage`, interactively, in the vault the item landed in. A wrong routing call costs one misfiled note; a wrong filing call costs an edit inside a vault, which is why only the first one happens here.

**It needs `para-shared/` beside it.** The two links below resolve once this folder is installed next to the other `para-*` skills, which is where the module README says to put it. In the repo they do not resolve, because the module deliberately sits outside `base/.claude/skills/` so a single-vault adopter does not carry its frontmatter every turn.
...
Run each script row from its own vault root. **Dry-run every one first, in both modes**, then in write mode pass `--write` only to a source whose dry run would add **20 items or fewer** to one vault. Above that, report the count and leave the import to a person: script output goes straight into `triage/` without passing the router, so a script with no gate of its own is the one thing here nothing can filter. **Full reasoning, and why an unattended run must be more cautious than a person: [references/gather.md](references/gather.md).**

These scripts dedupe against their own central ledgers, so running them is idempotent. **An error is recorded and skipped, never fatal:** one expired auth must not cost the whole run. Their output is not routed by this skill, because the script already knows where its items go.

### Step 3: Fetch each mailbox once

Follow **[../para-shared/connectors.md](../para-shared/connectors.md)**, the same fetch protocol `/para-triage` uses: dispatch, query frame, thread normalisation, dedup, message list. Two differences, both because the caller is different:
...
**Count carry-over by distinct thread id, never by decision record.** A log written by a run that failed to group messages into threads holds one decision per *message*, so counting records reports a number several times the real one - and reports it as a fleet-sized problem when it is a modest one. Measured once: 137 records over 68 threads, with a single conversation carrying 15 separate routing decisions. The grouping rule is in the shared fetch protocol, but the reconcile reads logs it did not write, so it defends itself rather than trusting them.
...
In write mode: one note per routed thread into that vault's `triage/`, one ledger entry per thread whether routed or not, one run log. In preview: the run log only. **Note shape, ledger shape, run log shape and the idempotency rule: [references/staging.md](references/staging.md).**

Report as a table of vault, staged count and unrouted count, then name every source that errored and every carry-over thread Step 3 left outstanding. A run that staged nothing says so, rather than reporting nothing.
...
- `/para-triage` - the interactive half. It empties the `triage/` folder this skill fills, and skips its own source pull in any vault the registry lists as active.
