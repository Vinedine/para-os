...
0. **A CLAUDE.md template** ([`skeleton/CLAUDE.md.template`](skeleton/CLAUDE.md.template)): the base skeleton template with four deltas, all downstream of "no `actions.md`, and the reader is on an iPad" - the base `## Actions` section (checkbox rule, which-file routing, Obsidian Tasks markers) dropped entirely; an `## iPad access and rendering` section added for the pipeline and reader-access model; `resources/mds/` added to the PARA layout; and root `README.md` stated to sit above the PARA folders, out of the reader's view. Everything else (the PARA sorting test, Lifecycle, Archive hygiene, Filing and naming, Authoritative sources, Language, Memory, File formats, Integration scripts, Do not add) tracks the base template verbatim except where the no-actions rule or the shared-Drive reader forces a rewording. A vault of this flavor follows this template, not `base/CLAUDE.md.template`.
