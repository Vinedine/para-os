...
- Every person holding a card in `areas/network/` is linked rather than merely named on first mention in each live-bucket file (exempting the vault's principals outside the root README), and their contact details appear only on the card
...
- Root README passes the Step 1.1 shape check, and no `{{...}}` template placeholder survives anywhere in the vault (exempting `resources/prompts/` and declared template folders)
